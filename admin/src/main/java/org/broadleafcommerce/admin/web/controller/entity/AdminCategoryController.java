/*
 * #%L BroadleafCommerce Admin Module %% Copyright (C) 2009 - 2013 Broadleaf Commerce %% Licensed
...
public class AdminCategoryController extends AdminBasicEntityController {
  ...
  public String viewEntityList(HttpServletRequest request, HttpServletResponse response,
      Model model, @PathVariable Map<String, String> pathVars,
    ...
    for (Category category : parentCategories) {
      if (((Status) category).getArchived() == 'N') {
        List<CategoryXref> xrefs = category.getAllChildCategoryXrefs();
        List<CategoryXref> xrefList = new ArrayList<CategoryXref>();
        for (CategoryXref xref : xrefs) {
          Category xrefCategory = xref.getSubCategory();
          xref.setSubCategory(category);
          if (((Status) xrefCategory).getArchived() == 'N') {
            xref.setSubCategory(xrefCategory);
            xrefList.add(xref);
          }
        }
        category.setAllChildCategoryXrefs(xrefList);
    ...
    mainActions.add(new EntityFormAction("CategoryTreeView").withButtonClass(
        "show-category-tree-view").withDisplayText("Category_Tree_View"));

    mainActions.add(new EntityFormAction("CategoryListView").withButtonClass(
        "show-category-list-view active").withDisplayText("Category_List_View"));
