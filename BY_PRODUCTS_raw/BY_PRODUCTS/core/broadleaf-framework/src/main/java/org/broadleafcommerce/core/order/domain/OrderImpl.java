...
public class OrderImpl implements Order, AdminMainEntity, CurrencyCodeIdentifiable, Previewable {
    ...
    @Column(name = "FEEDBACK", length = Integer.MAX_VALUE - 1)
    @AdminPresentation(friendlyName = "Feedback", 
    	group = "Feedback",
    	tab = "Feedback",
    	tabOrder = 8000,
    	largeEntry = true,
    ...
    @OneToMany(fetch = FetchType.EAGER, mappedBy = "order", targetEntity = FulfillmentGroupImpl.class, cascade = {CascadeType.ALL})
    @Cache(usage = CacheConcurrencyStrategy.NONSTRICT_READ_WRITE, region="blOrderElements")
