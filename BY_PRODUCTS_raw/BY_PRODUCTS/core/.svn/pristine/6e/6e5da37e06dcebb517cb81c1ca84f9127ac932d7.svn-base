 ...
 *
 * User: Elbert Bautista Date: 4/18/12
 */
@XmlRootElement(name = "customer")
@XmlAccessorType(value = XmlAccessType.FIELD)
public class CustomerWrapper extends BaseWrapper implements
		APIWrapper<Customer>, APIUnwrapper<Customer> {
	...
	public void wrapDetails(Customer model, HttpServletRequest request) {
		...
		if (model.getCustomerAttributes() != null
				&& !model.getCustomerAttributes().isEmpty()) {
			Map<String, CustomerAttribute> itemAttributes = model
			...
			for (String key : keys) {
				...
				customerAttributeWrapper.wrapDetails(itemAttributes.get(key),
						request);
	...
	public Customer unwrap(HttpServletRequest request,
		...
		if (customerAttributes != null) {
			for (CustomerAttributeWrapper customerAttributeWrapper : customerAttributes) {
				CustomerAttribute attribute = customerAttributeWrapper.unwrap(
						request, context);
				attribute.setCustomer(customer);
				customer.getCustomerAttributes().put(attribute.getName(),
						attribute);
			}
		}
		return customer;
